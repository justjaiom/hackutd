		...
		switch (section) {
			...
			case 'knowledge':
				return (
					...
					{knowledgeSub === 'repos' && (
						<div className="rounded-lg border border-gray-200 bg-gray-50 p-4 min-h-64">
							<p className="text-sm text-gray-600">Link GitHub / GitLab repositories, show commit health, etc.</p>
						</div>
					)}
					{knowledgeSub === 'architecture' && (
						<div className="rounded-lg border border-gray-200 bg-gray-50 p-4 min-h-64">
							<p className="text-sm text-gray-600">Document system architecture diagrams, services, and integrations.</p>
						</div>
					)}
				</div>
				)
			case 'meetings':
				return (
					<div className="space-y-6">
						...
						<div className="rounded-lg border border-gray-200 bg-gray-50 p-4 min-h-64">
							{meetingsSub === 'recordings' && <p className="text-sm text-gray-600">Store & browse meeting recordings.</p>}
							{meetingsSub === 'transcripts' && <p className="text-sm text-gray-600">View AI-generated transcripts & notes.</p>}
							{meetingsSub === 'actions' && <p className="text-sm text-gray-600">Track follow-up action items extracted from meetings.</p>}
							{meetingsSub === 'decisions' && <p className="text-sm text-gray-600">Log architectural & product decisions with rationale.</p>}
						</div>
					</div>
				)
			case 'work':
